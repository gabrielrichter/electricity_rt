# title: 5.regressions_month_brazil.py
...
for i in region_lst:
    region_dic[i] = pd.read_pickle(os.path.join(output, f'results_cluster_2020_{i}_month.pkl'))
    region_dic[i] = region_dic[i][['variable', 'beta']]
    region_dic[i]['date'] = pd.date_range(start='2020-01-01', end='2020-08-31', freq='M')
...
df_results.to_pickle(os.path.join(output, 'results_cluster_2020_brazil_month.pkl'))
