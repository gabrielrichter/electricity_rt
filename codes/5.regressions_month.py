...
for i in region_lst:
    ...
    x_interactions = patsy.dmatrix('C(df_subset_2020.index.month):C(df_subset_2020.index.year) - 1',
                                   data=df_subset_2020,
    ...
    #
    # beta_mean = results.loc[:1, 'beta'].mean()
    ...
    #
    # results.to_csv(os.path.join(output, f'results_2020_{region_name}_month.csv'), index=False)
    # results.to_pickle(os.path.join(output, f'results_2020_{region_name}_month.pkl'))
    ...
    beta_mean = results_cluster.loc[:1, 'beta'].mean()
    ...
    results_cluster.to_pickle(os.path.join(output, f'results_cluster_2020_{region_name}_month.pkl'))
